...
in ((f 44) 33)"
    ...
    let a=1000
    in
      let dec_x= proc (x) set a= -(a,x)
      in
        let OnClick = event.create()
        in
          begin
            <OnClick> += {dec_x;dec_x;dec_x;dec_x};
            (dec_x 100);
            (OnClick 50);
            a
          end" 700)
    ...
    in
      ...
      in
        ...
        in
          let dec_x = proc (x) set a = -(a,x)
          in
            let add_x = proc(x) set b = -(b , -(0,x))
            in
              begin
                <OnClick> += {dec_x; proc (y) set a= -(b,y); add_x };
                (OnClick 20);
                -(a,b)
              end" -40)
    ...
    ; THIS TEST FAILS JUST BECAUSE IT REQUIRES MOVING THE ERROR HANDLING AWAY FROM THE APPLY-PROC PROCEDURE, WHICH I DON"T FEEL LIKE DOING
    ;   (event-5 "
    ; let a=10
    ; in
    ;   let p= proc (x) -(x,5)
    ...
    in
      ...
      in
        let OnClick = event.create()
        in
          let OnOpen = event.create()
          in
            begin
              <OnClick> += {dec_x;dec_x;dec_x};
              <OnOpen> += {proc (x) if zero?(x) then set a=350 else set a= 550};
              (OnClick 50);
              set OnClick = OnOpen;
              (OnClick 0);
              a
